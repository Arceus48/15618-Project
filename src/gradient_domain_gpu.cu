...
__global__ void reduce(double* g_idata, double* g_odata, int total_num) {
...
__global__ void init_kernel(double* gradX, double* gradY, double* D,
  ...
  B[linear_idx] = 0.0;
  if (row >= EDGE_WIDTH && row < nrow - EDGE_WIDTH && col >= EDGE_WIDTH &&
      col < ncol - EDGE_WIDTH) {
    B[linear_idx] = 1.0;
  }
...
__global__ void inte_stage1(double* d, double* q, double* tmp, int nrow,
                            int ncol) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int linear_idx = row * ncol + col;
...
__global__ void inte_stage2(double* B, double* d, double* tmp, double* output,
                            double* q, double* r, double ita, int nrow,
                            int ncol) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int linear_idx = row * ncol + col;
...
__global__ void inte_stage3(double* d, double* tmp, double* r, double beta,
                            int nrow, int ncol) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int linear_idx = row * ncol + col;
...
void printGrad(double* fused, int nrow, int ncol) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    cudaMemcpy(fuse_gradX[i], fused + 2 * i * nrow * ncol,
               nrow * ncol * sizeof(double), cudaMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    cudaMemcpy(argb_gpu[i], argb[i], nrow * ncol * sizeof(double),
